...
def export_final_result_to_txt():
    ...
    with open(fileName,"a+") as fic :
        ...
        fic.write('Moyenne du nombre de bus réparés : {} \n'.format(sum(listNumberBusesRepaired)/len(listNumberBusesRepaired)))
        fic.write('Moyenne du temps moyen attente file controle : {} \n'.format(sum(listAverageWaitingTimeControl)/len(listAverageWaitingTimeControl)))
        fic.write('Moyenne de la taille moyenne file controle : {}\n'.format(sum(listAverageSizeControlQueue)/len(listAverageSizeControlQueue)))
        fic.write('moyenne du temps moyen attente file réparation : {} \n'.format(sum(listAverageWaitingTimeRepair)/len(listAverageWaitingTimeRepair)))
        fic.write('Moyenne de la taille moyenne file réparation : {}\n'.format(sum(listAverageSizeRepairQueue)/len(listAverageSizeRepairQueue)))
        fic.write('Moyenne du taux d\'utilisation du centre de réparation : {}\n\n'.format(sum(listRateUserRepair)/len(listRateUserRepair)))
...
fileName = "SimulatorRepairCenter-{},{}.txt".format(stopBusNumer,repeatSimulation)
billBookExport = []
